...
const SheepAvatarScreen: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F4971D] pb-24 px-4 pt-4 flex flex-col">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="flex items-center gap-0.5">
                        {[1, 2, 3, 4].map((i) => (
                          <Leaf
                            key={i}
                            className={`h-3 w-3 ${i <= 5 - p.carbonScore ? "text-green-400" : "text-white/20"}`}
                          />
                        ))}
                      </span>
                    </div>
      ...
      <button onClick={onBack} className="w-full mt-4 bg-[#5a3d1c] text-white font-serif font-bold py-4 rounded-xl">
        {t("CONFIRM")}
      </button>
    </div>
  );
