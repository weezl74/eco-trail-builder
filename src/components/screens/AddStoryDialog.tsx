...
const baseCategories = [
  ...
  },
  {
    ...
    items: [
      { tag: "#WalkItOut", text: "Walked the pupils to school every day this week." },
...
const AddStoryDialog: React.FC<Props> = ({ open, onOpenChange, onPosted, isBusiness = false }) => {
  ...
  // ✅ LOAD ALREADY USED STORIES FROM API (instead of Supabase)
  useEffect(() => {
    if (!open || !user) return;
    ...
    (async () => {
      try {
        ...
        const myTags = (data || []).filter((s: any) => s.user_id === user.id).map((s: any) => s.title);

        setUsedTags(new Set(myTags));
      } catch (err) {
        console.error("❌ load used stories failed:", err);
      }
    })();
  ...
  // ✅ CREATE STORY VIA API (THIS IS THE KEY CHANGE)
  const share = async (tag: string, text: string) => {
    if (!user) {
      toast({
        title: t("Please sign in"),
        description: t("You need to be signed in to share a story."),
      });
      return;
    }
    ...
    if (usedTags.has(tag)) {
      toast({
        title: t("Already shared"),
        description: t("You can only post each story once."),
      });
      return;
    }
            ...
            {cat.items.map((it, i) => {
              ...
              return (
                ...
                >
                  ...
                  {used && <p className="text-white/60 text-xs mt-1">{t("Already shared")}</p>}

                  {submittingTag === it.tag && <p className="text-white/60 text-xs mt-1">{t("Sharing…")}</p>}
                </button>
              );
            })}
          </div>
