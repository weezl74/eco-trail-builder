        ...
        <div className="bg-[#1f1f1f] rounded-2xl px-3 py-2 flex gap-2">
          {(['all','activated','progress'] as Tab[]).map((tb) => (
            <button
              key={tb}
              onClick={() => setTab(tb)}
              className={`px-3 py-1 rounded-xl font-serif font-bold text-white text-sm ${tab===tb ? 'border border-white' : 'opacity-80'}`}
            >
            ...
            <div key={p.id} className="bg-[#1f1f1f] rounded-2xl p-5 text-white">
              ...
              <div className="border border-white/40 rounded-xl mt-4 p-3 grid grid-cols-2 gap-2 text-sm font-serif">
                <div className="border-r border-white/40 pr-2">
                  ...
                  <p className="flex items-center gap-2">🟢 {t('Wool Points')}: {p.wool_points ?? 0}</p>
                </div>
                <div className="pl-2">
                  <p className="text-center font-bold mb-1">{t('Global Change')}</p>
                  <p className="text-center text-xs opacity-80">{t('(Estimated Values)')}</p>
                  <p className="flex items-center gap-2 mt-1"><span className="text-yellow-400 text-lg">£</span> {t('Savings')} £{p.money_saved ?? 0}</p>
                  <p className="flex items-center gap-2 mt-1"><span className="text-red-400 text-xs font-mono">CO₂e</span> {t('Carbon Saved')} {p.co2_saved ?? 0} kg CO₂</p>
                  {Number(p.water_saved ?? 0) > 0 && (
                    <p className="flex items-center gap-2 mt-1"><span className="text-blue-300 text-xs font-mono">H₂O</span> {t('Water Saved')} {p.water_saved} L</p>
                  )}
                </div>
              </div>
