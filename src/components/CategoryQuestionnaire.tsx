  ...
  buildings: [
    {
      id: "home-size",
      question:
        "What is the floor area of your home? (Enter square footage below — or pick by number of rooms if you don't know)",
      options: [
...
const CategoryQuestionnaire: React.FC<CategoryQuestionnaireProps> = ({ category, onComplete, onClose, user }) => {
        ...
        <CardContent>
          <div className="mb-6">
            ...
            <div className="space-y-3">
              <p className="text-sm text-slate-400 mb-3">Select one option:</p>
              <RadioGroup value={(answers[currentQuestion.id] || [])[0] || ""} onValueChange={(v) => handleAnswer(v)}>
                {currentQuestion.options.map((option) => (
                  <div key={option.value} className="flex items-center space-x-3">
                    <RadioGroupItem id={option.value} value={option.value} className="border-slate-500 text-blue-500" />
                    <Label htmlFor={option.value} className="text-slate-300 cursor-pointer flex-1">
                      {option.label}
                      <span className="text-xs text-slate-500 ml-2">
                        ({option.impact > 0 ? "+" : ""}
                        {option.impact} kg CO₂/year)
                      </span>
                    </Label>
                  </div>
                ))}
              ...
              {currentQuestion.allowCustom && (
                <div className="mt-6 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <Label className="text-slate-300 font-medium">Custom Value</Label>
                  </div>
                  <p className="text-sm text-slate-400 mb-3">{currentQuestion.customLabel}</p>
                  <div className="flex gap-2">
                    ...
                    />
                    <span className="text-slate-400 text-sm flex items-center px-2">{currentQuestion.customUnit}</span>
                  </div>
